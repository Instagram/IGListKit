...
- (void)reloadDataWithCollectionView:(UICollectionView *)collectionView itemUpdateBlock:(IGListReloadUpdateBlock)itemUpdateBlock completion:(IGListUpdatingCompletion)completion {
    itemUpdateBlock();
    [self synchronousReloadDataWithCollectionView:collectionView];
    if (completion) {
        completion(YES);
