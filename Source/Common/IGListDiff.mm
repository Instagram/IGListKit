...
static id IGListDiffing(BOOL returnIndexPaths,
    ...
    // Helper for returning the correct Result
    id (^makeResult)() = ^id () {
    ...
    // Guard against empty arrays that don't require diffing
    if (oldCount == 0) {
        // Insert every new value (with no deletes)
    ...
    if (newCount == 0) {
        // Delete every old value (with no inserts)
