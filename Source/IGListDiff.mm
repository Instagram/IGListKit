...
static BOOL IGListRecordIsInsert(const IGListRecord r) {
    return r.index == NSNotFound;
}
...
static id IGListDiffing(BOOL returnIndexPaths,
    ...
    for (NSInteger i = 0; i < newCount; i++) {
        ...
        if (originalIndex < oldCount) {
            ...
            switch (option) {
                ...
                case IGListDiffEquality:
                    // use -[NSObject isEqual:] between both version of data to see if anything has changed
        ...
        if (originalIndex != NSNotFound
            && entry->newCounter > 0
            && entry->oldCounter > 0) {
            // if an item occurs once in the new and old array, it is unique
            // replace the entry in na w/ the original line number
            // replace the entry in oa w/ the new line number
    ...
    // track offsets from inserted items to calculate where items have moved
    runningOffset = 0;
    ...
    for (NSInteger i = 0; i < newCount; i++) {
        ...
        // add to inserts if the old index is NSNotFound
        if (IGListRecordIsInsert(record)) {
